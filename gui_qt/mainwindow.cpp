...
//preprocessors needed to read from memory address in DE1-SoC
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
...
void MainWindow::setupRealtimeDataDemo(QCustomPlot *customPlot)
{
  ...
  // enable mouse wheel to zoom in and out of axis
  ...
  */
  ...
  customPlot->addGraph();
  customPlot->graph(3)->setPen(QPen(QColor(0, 180, 100)));

  QSharedPointer<QCPAxisTickerTime> timeTicker(new QCPAxisTickerTime);
  timeTicker->setTimeFormat("%h:%m:%s");
  customPlot->xAxis->setRange(-0.005, 0.005);
  customPlot->axisRect()->setupFullAxesBox();
...
//  connect(&dataTimer, SIGNAL(timeout()), this, SLOT(realtimeDataSlot()));
  setupIncomingData();
  connect(&dataTimer, SIGNAL(timeout()), this, SLOT(readNewData2()));
...
void MainWindow::setupIncomingData(){
    qDebug() << "initialiing connection to read data from memory";
    ...
    status_addr = (void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + OSCOPE_STATUS_BASE ) & ( unsigned long)( HW_REGS_MASK ) ));
    channel_1_virtual_addr = (void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_1_MEM_BASE ) & ( unsigned long)( HW_REGS_MASK ) ));
    channel_2_virtual_addr = (void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_2_MEM_BASE ) & ( unsigned long)( HW_REGS_MASK ) ));



}

void MainWindow::readNewData2(){
    QVector<double> channel1_voltage(NUM_SAMPLES + 2), channel2_voltage(NUM_SAMPLES + 2), channel1_time(NUM_SAMPLES + 2), channel2_time(NUM_SAMPLES + 2);

    unsigned int val;
    uint32_t time_base1, time_base2, trigger1, trigger2;
    double freq_channel1, freq_channel2;
    double time_period1, time_period2;
    //**********read statuus register*************//
    val = *(uint32_t *)status_addr;
    printf("\n \n value in status reg: %X \n\n", val);
    time_base1 = val & 0b00000111000000000000000000000000;
    time_base1 = time_base1 >> 16;
    time_base2 = val & 0b00111000000000000000000000000000;
    time_base2 = time_base2 >> 19;

    //trigger 1
    trigger1 = val & 0x000000FF;
    double trigger1_level = (2* 2.048 * trigger1)/255;
    trigger1_level = 2.5 - trigger1_level;

    //trigger 2
    trigger2 = val & 0x0000FF00;
    trigger2 = trigger2 >> 8;
    double trigger2_level = (2 * 2.048 * trigger2)/255;
    trigger2_level = 2.5 - trigger2_level;


    switch(time_base1){
        case 0:
            freq_channel1 = 12.5 * pow(10, 6);
            break;
        case 1:
            freq_channel1 = 6.25 * pow(10, 6);
            break;
        case 2:
            freq_channel1 = 3.125 * pow(10, 6);
            break;
        case 3:
            freq_channel1 = 1.625 * pow(10, 6);
            break;
        case 4:
            freq_channel1 = 788 * pow(10, 3);
            break;
        case 5:
            freq_channel1 = 380 * pow(10, 3);
            break;
        case 6:
            freq_channel1 = 198 * pow(10, 3);
            break;
        case 7:
            freq_channel1 = 98 * pow(10, 3);
            break;
        default:
            break;
    }

    time_period1 = 1/freq_channel1;
    printf("time base ch1: %f\n", time_base1);
    printf("freq ch1: %f\n", freq_channel1);



    //***********read channel 1 memory sample 1*********//
...
//    printf("\n \n value @ addr 0 to 3: %X \n\n", val);
    ...
    for (int i = 0; i < NUM_SAMPLES - 1; i++){
        ...
        channel_1_virtual_addr =(void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_1_MEM_BASE + i) & ( unsigned long)( HW_REGS_MASK ) ));
        val = *(uint8_t *)channel_1_virtual_addr;
        channel1_voltage[i] = val;
        channel1_voltage[i] = ((channel1_voltage[i]/255) *  2.489) + 1;
        channel1_voltage[i] = 2.5 - channel1_voltage[i];
        channel1_time[i] = i*time_period1;
        if (i < 8){
            printf("sample: %d, voltage: %f, time: %f\n", i, channel1_voltage[i], channel1_time[i]);
        }
        if (i == NUM_SAMPLES - 2){
            printf("last sample voltage: %f, last sample time: %f\n", channel1_voltage[i], channel1_time[i]);
        }
//        printf("voltage channel1: %f", channel1_voltage[i]);
    }


    QVector<double> trig1x(2), trig1y(2);
    trig1x[0] = -100;
    trig1y[0] = trigger1_level;
    trig1x[1] = 100;
    trig1y[1] = trigger1_level;

    QVector<double> trig2x(2), trig2y(2);
    trig2x[0] = -100;
    trig2y[0] = trigger2_level;
    trig2x[1] = 100;
    trig2y[1] = trigger2_level;

    // allow interpolation of samples
    QCPItemTracer *phaseTracer = new QCPItemTracer(ui->customPlot);
    phaseTracer->setInterpolating(true);

    ui->customPlot->graph(0)->data()->clear();
    ui->customPlot->graph(0)->addData(channel1_time, channel1_voltage, true);
    ui->customPlot->graph(1)->data()->clear();
    ui->customPlot->graph(1)->addData(trig2x, trig2y, true);
    ui->customPlot->graph(2)->data()->clear();
    ui->customPlot->graph(2)->addData(trig1x, trig1y, true);
    ui->customPlot->replot();



//    //add 4 to get address 4
//    channel_1_virtual_addr =(void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_1_MEM_BASE + 4) & ( unsigned long)( HW_REGS_MASK ) ));
//    val = *(uint32_t *)channel_1_virtual_addr;
//    printf("\n \n value @ addr 4 to 7: %X \n\n", val);

//    //get last address why not 102450????? 102399
//    channel_1_virtual_addr =(void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_1_MEM_BASE + 102399) & ( unsigned long)( HW_REGS_MASK ) ));
//    val = *(uint8_t *)channel_1_virtual_addr;
//    printf("\n \n value @ addr 102399: %X \n\n", val);
...
//    //add 4 to get address 4
//    channel_2_virtual_addr =(void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_2_MEM_BASE + 4) & ( unsigned long)( HW_REGS_MASK ) ));
//    val = *(uint32_t *)channel_2_virtual_addr;
//    printf("\n \n value @ addr 4 to 7: %X \n\n", val);
...
//    //get last address
//    channel_2_virtual_addr =(void*)((char*)virtual_base + ( ( unsigned long  )( ALT_LWFPGASLVS_OFST + CHANNEL_2_MEM_BASE + 102399) & ( unsigned long)( HW_REGS_MASK ) ));
...
{
    // open test data text file
    QFile file("/usr/local/testData.txt");
    ...
    QVector<double> x(NUM_SAMPLES + 2), y(NUM_SAMPLES + 2);
    QVector<double> x2(NUM_SAMPLES + 2), y2(NUM_SAMPLES + 2);
    double x_window_range = 4;
    double num_samples = NUM_SAMPLES;
    double constantFraction = x_window_range/num_samples;
    int sampleCounter = 0;
    float timeBase, trigger1_level, trigger2_level;
    s >> timeBase >> trigger1_level >> trigger2_level;
//    qDebug() << "timebase: " << timeBase << " trigger1: " << trigger1_level << " trigger2: " << trigger2_level;
    while(!s.atEnd()){
        float voltage, voltage2;
        s >> voltage;
        s >> voltage2;
        x[sampleCounter] = sampleCounter * constantFraction;
        float r3 = -0.3 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.6)));
        float r4 = -0.3 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.6)));
        y[sampleCounter] = voltage + r3;
        y2[sampleCounter] = voltage2 + r4;
        qDebug() << "x1: " << x[sampleCounter] << " y1: " << y[sampleCounter];
        qDebug() << "x2: " << x2[sampleCounter] << " y2: " << y2[sampleCounter];
        sampleCounter++;
    }
    QString timeBase_str= QString::number(timeBase);
    QString trigger1_str = QString::number(trigger1_level);
    QString trigger2_str = QString::number(trigger2_level);
    ui->statusBar->showMessage("Time base: " + timeBase_str + " seconds per division, trigger1: " + trigger1_str + ", trigger2: " + trigger2_str);
...
//    ui->customPlot->graph(1)->setPen(QPen(QColor(255, 110, 40)));
    ui->customPlot->graph(1)->addData(x, y2, true);
    ui->customPlot->graph(2)->data()->clear();
    ui->customPlot->graph(2)->addData(trig1x, trig1y, true);
    ui->customPlot->graph(3)->data()->clear();
    ui->customPlot->graph(3)->addData(trig2x, trig2y, true);
    ui->customPlot->replot();
    file.close();
}
...
{
    ui->customPlot->yAxis->scaleRange(1.15, ui->customPlot->xAxis->range().center());
    ui->customPlot->replot();
}

void MainWindow::on_pushButton_2_clicked()
{
    ui->customPlot->yAxis->scaleRange(0.85, ui->customPlot->xAxis->range().center());
    ui->customPlot->replot();
}
...
{
    ui->customPlot->xAxis->scaleRange(1.15, ui->customPlot->xAxis->range().center());
    ui->customPlot->replot();
}
